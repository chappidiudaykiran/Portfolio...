  ...
  return (
    <section id="home" className="relative z-10 flex min-h-screen items-center justify-center overflow-hidden px-5 pb-8 pt-20 sm:px-6 md:px-14">
      ...
      <div className="relative mx-auto max-w-4xl text-center">
        {/* Status badge */}
        <div className="mb-8 inline-flex items-center gap-2.5 rounded-full border border-accent/15 bg-accent/[0.04] px-5 py-2.5 opacity-0 animate-fade-in" style={{ animationDelay: "0.2s", animationFillMode: "forwards" }}>
          <span className="relative flex h-2 w-2">
            <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-emerald-400 opacity-75" />
            <span className="relative inline-flex h-2 w-2 rounded-full bg-emerald-400" />
          </span>
          <span className="text-[11px] font-medium uppercase tracking-[0.18em] text-accent sm:text-xs">
            Open to Opportunities
          </span>
        </div>

        {/* Name */}
        <h1 className="mb-4 opacity-0 animate-fade-in-up font-display text-4xl font-bold tracking-tight sm:text-6xl md:text-7xl" style={{ animationDelay: "0.4s" }}>
        ...
        <div className="mb-6 flex items-center justify-center opacity-0 animate-fade-in-up" style={{ animationDelay: "0.6s" }}>
          ...
          <p className="font-display text-base font-medium tracking-wide text-apptext/50 sm:text-lg md:text-xl">
            {displayed}
        ...
        {/* Description */}
        <p className="mx-auto mb-9 max-w-2xl text-[15px] leading-7 text-apptext/35 opacity-0 animate-fade-in-up sm:text-base sm:leading-8 md:text-lg" style={{ animationDelay: "0.8s" }}>
        ...
        <div className="mb-10 flex flex-wrap justify-center gap-3 opacity-0 animate-fade-in-up" style={{ animationDelay: "1s" }}>
          {[
            { label: "MERN Stack", icon: "🚀", glow: "#00f0ff" },
            { label: "Hackathon Winner", icon: "🏆", glow: "#f59e0b" },
            { label: "150+ LeetCode", icon: "⚡", glow: "#10b981" },
            { label: "CGPA 8.84", icon: "🎓", glow: "#7c3aed" },
          ].map((b) => (
            <span
              key={b.label}
              className="group flex items-center gap-2 rounded-full border border-white/[0.06] bg-white/[0.02] px-4 py-2 text-[11px] font-medium tracking-wide text-apptext/50 transition-all duration-400 hover:border-accent/20 hover:text-apptext/80 sm:text-xs"
              style={{ "--glow": b.glow }}
            >
        ...
        <div className="flex flex-col items-stretch justify-center gap-3 opacity-0 animate-fade-in-up sm:flex-row sm:gap-4" style={{ animationDelay: "1.2s" }}>
          <button
            className="group relative w-full overflow-hidden rounded-xl px-8 py-3.5 text-xs font-bold uppercase tracking-[0.18em] text-appbg transition-all duration-300 hover:shadow-[0_0_30px_rgba(0,240,255,0.2)] sm:w-auto sm:px-10"
          ...
          <button
            className="w-full rounded-xl border border-accent/15 bg-accent/[0.03] px-8 py-3.5 text-xs font-bold uppercase tracking-[0.18em] text-accent/70 transition-all duration-300 hover:border-accent/30 hover:bg-accent/[0.06] hover:text-accent hover:shadow-[0_0_20px_rgba(0,240,255,0.06)] sm:w-auto sm:px-10"
        ...
        <div className="mt-10 flex items-center justify-center gap-4 opacity-0 animate-fade-in-up" style={{ animationDelay: "1.4s" }}>
          {[
            ...
            { label: "Email", href: "mailto:udaykiran24689@gmail.com", icon: (
